...
<body class="bg-gray-100 min-h-screen">
    <div class="max-w-6xl mx-auto py-6 px-4">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold">Daily Timeline</h1>
            <p class="text-gray-600">Smart Fountain uses three fixed blocks: Day, Evening, and Night. Edit the time and scene for each block.</p>
        </div>

        <div class="mb-4 rounded border border-blue-200 bg-blue-50 px-4 py-3 text-blue-800">
            Enabled blocks cannot overlap. A block may end at the same time the next block starts.
        </div>
                ...
                <div class="rounded-lg bg-white p-5 shadow">
                    <div class="mb-4 flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ ucfirst($schedule->period_key) }} timeline block</p>
                        </div>
                    ...
                    <div class="mb-4 space-y-2 rounded border border-gray-200 bg-gray-50 px-4 py-3 text-sm">
                        ...
                        <p><strong>Time:</strong> {{ substr($schedule->start_time, 0, 5) }} → {{ substr($schedule->end_time, 0, 5) }}</p>
                        <p><strong>Scene:</strong> {{ $schedule->startScene?->name ?? 'Missing scene' }}</p>
