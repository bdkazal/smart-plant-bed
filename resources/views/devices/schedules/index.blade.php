...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .phone-frame {
            max-width: 460px; margin: 0 auto; padding: 18px; border-radius: 42px; background: rgba(255,255,255,.82);
            box-shadow: 0 30px 70px rgba(15,23,42,.14), inset 0 0 0 1px rgba(255,255,255,.9);
        }
        .app-screen { overflow: hidden; border-radius: 30px; background: linear-gradient(180deg, rgba(255,255,255,.96), rgba(238,243,248,.94)); border: 1px solid rgba(226,232,240,.9); }
        ...
        .notice { margin-bottom: 14px; border-radius: 16px; padding: 12px 14px; font-size: 14px; font-weight: 650; background: #dcfce7; color: #166534; }

        .add-button {
            display: flex; align-items: center; justify-content: center; width: 100%; min-height: 50px; margin-bottom: 14px;
            border-radius: 999px; background: linear-gradient(135deg, #2aa8ff, #0877ef); color: #fff; font-size: 14px;
            font-weight: 900; text-transform: uppercase; text-decoration: none; box-shadow: 0 16px 30px rgba(37,99,235,.35);
        }

        .empty-card, .schedule-card {
            margin-bottom: 13px; border-radius: 22px; border: 1px solid rgba(255,255,255,.86); background: rgba(255,255,255,.78);
            box-shadow: 0 18px 38px rgba(15,23,42,.09), inset 0 1px 0 rgba(255,255,255,.86); backdrop-filter: blur(16px); padding: 16px;
        }

        .schedule-top { display: flex; justify-content: space-between; gap: 12px; align-items: flex-start; margin-bottom: 12px; }
        .day-name { margin: 0; font-size: 20px; font-weight: 900; letter-spacing: -.04em; }
        .status-pill { border-radius: 999px; padding: 7px 10px; font-size: 12px; font-weight: 850; text-transform: uppercase; }
        .status-pill.on { background: #dcfce7; color: #166534; }
        .status-pill.off { background: #e5e7eb; color: #475569; }

        .time-line { display: flex; align-items: baseline; gap: 8px; margin: 12px 0 10px; }
        .time-value { font-size: 36px; line-height: 1; font-weight: 950; letter-spacing: -.06em; }
        .time-label { color: var(--text-muted); font-size: 13px; font-weight: 800; }

        .detail-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 14px; }
        .detail-box { border-radius: 16px; background: #f8fafc; border: 1px solid rgba(148,163,184,.22); padding: 12px; }
        .detail-label { margin: 0 0 4px; color: var(--text-muted); font-size: 12px; font-weight: 800; }
        .detail-value { margin: 0; font-size: 15px; font-weight: 900; }

        .actions { display: flex; flex-wrap: wrap; gap: 8px; }
        .btn { border: 0; border-radius: 999px; padding: 10px 14px; cursor: pointer; font-size: 13px; font-weight: 850; text-decoration: none; display: inline-flex; align-items: center; justify-content: center; }
        ...
        <main class="phone-frame">
            <div class="app-screen">
                <div class="app-content">
                    <section class="hero">
                        <p class="eyebrow">Plant Bed Watering</p>
