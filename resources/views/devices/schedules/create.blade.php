...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .notice { margin-bottom: 14px; border-radius: 16px; padding: 12px 14px; font-size: 14px; font-weight: 650; background: #fee2e2; color: #991b1b; }

        .card {
            margin-bottom: 14px; border-radius: 22px; border: 1px solid rgba(255,255,255,.86); background: rgba(255,255,255,.78);
            box-shadow: 0 18px 38px rgba(15,23,42,.09), inset 0 1px 0 rgba(255,255,255,.86); backdrop-filter: blur(16px); padding: 16px;
        }

        .field { margin-bottom: 14px; }
        .label { display: block; margin-bottom: 6px; font-size: 13px; font-weight: 850; }
        .input, .select {
            width: 100%; min-height: 48px; border-radius: 16px; border: 1px solid rgba(148,163,184,.45);
            background: rgba(255,255,255,.92); padding: 0 14px; color: var(--text-main); font-size: 15px; font-weight: 700; outline: none;
        }
        .input:focus, .select:focus { border-color: var(--blue); box-shadow: 0 0 0 4px rgba(22,135,249,.14); }
        .hint { margin: 7px 0 0; color: var(--text-muted); font-size: 12px; line-height: 1.4; }

        .toggle-row { display: flex; align-items: center; gap: 10px; border-radius: 16px; background: #f8fafc; padding: 12px; border: 1px solid rgba(148,163,184,.22); font-weight: 750; }
        .save-button {
            width: 100%; min-height: 50px; border: 0; border-radius: 999px; cursor: pointer;
            background: linear-gradient(135deg, #2aa8ff, #0877ef); color: #fff; font-size: 14px; font-weight: 900; text-transform: uppercase;
            ...
            <div class="app-screen">
                <div class="app-content">
                    <section class="hero">
                        ...
                        <p class="subtitle">Add a timed watering run for {{ $device->name }}. Timezone: {{ $device->timezone ?? 'Asia/Dhaka' }}.</p>
                    </section>

                    <form action="{{ route('devices.schedules.store', $device) }}" method="POST">
                        @csrf
                        ...
                        <section class="card">
                            ...
                            <div class="field">
                                ...
                                <input type="time" name="time_of_day" id="time_of_day" value="{{ old('time_of_day') }}" class="input" required>
                            ...
                            <label class="toggle-row" for="is_enabled">
                                <input type="checkbox" name="is_enabled" id="is_enabled" value="1" @checked(old('is_enabled', true))>
                                <span>Enable this schedule immediately</span>
                            </label>
                        </section>

                        <button type="submit" class="save-button">Create Schedule</button>
                    </form>
                </div>
            </div>
        </main>
